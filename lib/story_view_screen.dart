...
class _StoryViewScreenState extends State<StoryViewScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _animationController.addStatusListener((status) { if (status == AnimationStatus.completed) _onTapNext(); });
    _playStoryAtIndex(0);
  }

  @override
  void dispose() { _animationController.dispose(); _pageController.dispose(); super.dispose(); }
  ...
  void _playStoryAtIndex(int index) {
    ...
    _animationController.stop(); _animationController.reset();
  ...
  void _onPageChanged(int index) { setState(() => _currentIndex = index); _playStoryAtIndex(index); }
  void _onTapDown(TapDownDetails details, BuildContext context) { final screenWidth = MediaQuery.of(context).size.width; if (details.globalPosition.dx < screenWidth / 3) _onTapPrevious(); else _onTapNext(); }
  void _onTapNext() { if (_currentIndex < widget.storyDocs.length - 1) _pageController.nextPage(duration: const Duration(milliseconds: 250), curve: Curves.easeIn); else Navigator.of(context).pop(); }
  void _onTapPrevious() { if (_currentIndex > 0) _pageController.previousPage(duration: const Duration(milliseconds: 250), curve: Curves.easeIn); else _playStoryAtIndex(0); }
  ...
  // SỬA: HOÀN THIỆN LOGIC LIKE VÀ THÔNG BÁO
  void _toggleLike() async {
    ...
    try { await batch.commit(); } catch(e) { setState(() => _isLikedMap[_currentIndex] = isCurrentlyLiked); }
  }

  // SỬA: HOÀN THIỆN LOGIC NHẮN TIN
  ...
  void _showShareSheet() { /* Placeholder */ ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Chức năng đang phát triển'))); }

  void _deleteStory() async {
    _pauseStory();
    final confirm = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(backgroundColor: darkSurface, title: const Text('Xóa tin', style: TextStyle(color: Colors.white)), content: const Text('Bạn có chắc chắn muốn xóa tin này vĩnh viễn không?', style: TextStyle(color: sonicSilver)), actions: [TextButton(child: const Text('Hủy', style: TextStyle(color: sonicSilver)), onPressed: () => Navigator.of(ctx).pop(false)), TextButton(child: const Text('Xóa', style: TextStyle(color: coralRed)), onPressed: () => Navigator.of(ctx).pop(true))]));
    if (confirm == true) {
      try {
        await _firestore.collection('stories').doc(widget.storyDocs[_currentIndex].id).delete();
        if (mounted) Navigator.of(context).pop();
      } catch (e) { if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Lỗi: Không thể xóa tin.'), backgroundColor: coralRed)); }
    }
    _resumeStory();
  }

  void _reportStory() { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Cảm ơn bạn đã báo cáo.'))); }
  void _unfollowUser() { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Đã bỏ theo dõi người dùng này.'))); }
  void _muteStories() { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Đã ẩn tin từ người dùng này.'))); }
  ...
  void _showStoryOptions() {
    ...
    showModalBottomSheet(context: context, backgroundColor: darkSurface, builder: (ctx) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
  ...
  void _showLikers() { _pauseStory(); showModalBottomSheet(context: context, isScrollControlled: true, backgroundColor: Colors.transparent, builder: (_) => StoryViewersScreen(storyId: widget.storyDocs[_currentIndex].id)).whenComplete(_resumeStory); }
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        ...
        child: Stack(children: [
          PageView.builder(controller: _pageController, onPageChanged: _onPageChanged, itemCount: widget.storyDocs.length, itemBuilder: (context, index) { final data = widget.storyDocs[index].data() as Map<String, dynamic>? ?? {}; return _StoryPageItem(storyData: data); }),
  ...
  Widget _buildStoryHeader() {
    return Row(children: [
      Container(padding: const EdgeInsets.all(1.5), decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.white.withOpacity(0.8), width: 1.5)), child: CircleAvatar(radius: 16, backgroundImage: (widget.avatarUrl != null ? NetworkImage(widget.avatarUrl!) : null), backgroundColor: darkSurface)),
      const SizedBox(width: 10), Text(widget.userName, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 15)),
      const SizedBox(width: 8), const Text('1h', style: TextStyle(color: sonicSilver)),
      const Spacer(),
  ...
  Widget _buildFixedContentWidget() {
    ...
    return Positioned.fill(child: SafeArea(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 10.0), child: Column(children: [
      Row(children: widget.storyDocs.asMap().entries.map((entry) { return Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 2), child: AnimatedBuilder(animation: _animationController, builder: (context, child) { return LinearProgressIndicator(value: (_currentIndex == entry.key) ? _animationController.value : ((_currentIndex > entry.key) ? 1.0 : 0.0), backgroundColor: Colors.white.withOpacity(0.5), valueColor: const AlwaysStoppedAnimation<Color>(Colors.white)); }))); }).toList()),
      const SizedBox(height: 8), _buildStoryHeader(), const Spacer(),
      Row(children: [
